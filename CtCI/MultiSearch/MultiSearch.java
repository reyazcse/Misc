...
public class MultiSearch {
	...
	private void findWords(String big, int index, Map<String, List<Integer>> lookup, TrieNode1 root) {
		int i=index;
		while (index < big.length()) {
			char value = big.charAt(index);
			...
			if(child.terminates)
				addWordToMap(lookup,big.substring(i, index+1), i);
			root = child;
			index++;
		}
