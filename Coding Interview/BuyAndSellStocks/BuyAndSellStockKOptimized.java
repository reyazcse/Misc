...
public class BuyAndSellStockKOptimized {
	public int maxProfit(int[] prices, int k) {
		...
		for(int i=1; i<=k; i++) {
			int maxDiff = table[i-1][0] - prices[0] ;  //maxDiff = table[i-1][m] + prices[j] - prices[m]. Since prices[j] is constant for a particular j,  
			for(int j=1; j<prices.length; j++) {	   // we do not consider j	
